<script lang="ts">
	 ...
	 * P126-02 — Case id for API calls from `getRouteCaseIdString` only (no direct `params.id` reads).
	 */
...
<CaseWorkspaceContentRegion testId="case-entities-page">
	<div class="flex flex-col min-h-0 flex-1 w-full">
		...
		<div class="px-3 sm:px-4 pt-3 sm:pt-4 max-w-[960px] mx-auto w-full min-h-0 overflow-auto flex-1">
			{#key routeCaseId}
				<CaseEntitiesPanel caseId={routeCaseId} caseEngineToken={$caseEngineToken ?? ''} />
			{/key}
		</div>
	</div>

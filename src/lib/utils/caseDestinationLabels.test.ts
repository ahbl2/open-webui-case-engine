...
describe('caseDestinationLabels (P78-16)', () => {
	it('matches case workspace shell tab labels for primary destinations', () => {
		...
		expect(CASE_DESTINATION_LABELS.entityIntelligence).toBe('Subjects & Assets');
	...
	it('preserves distinct meanings for Intelligence mode vs Subjects & Assets tab', () => {
		expect(CASE_DESTINATION_LABELS.intelligenceMode).toBe('Intelligence');
		expect(CASE_DESTINATION_LABELS.entityIntelligenceFocusDrillDown).toContain('Subjects & Assets');
	});
	...
	it('P78-18 — entry copy names Intelligence mode for handoff and Subjects default for shell title', () => {
		expect(INTELLIGENCE_ENTRY_COPY.chatHandoffLead.toLowerCase()).toContain('intelligence mode');
		expect(CASE_DESTINATION_TITLES.entityIntelligenceShellTabEntry.toLowerCase()).toContain('subjects mode');

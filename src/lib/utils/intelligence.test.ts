...
describe('detectIntelligenceIntent', () => {
	...
	it('builds deterministic handoff message', () => {
		expect(buildIntelligenceHandoffMessage('case-123')).toBe(
			'Cross-case analysis: open Subjects & Assets in Intelligence mode (search & Ask): /case/case-123/intelligence?mode=intelligence'
		);
	});
	...
	it('prefers handoff for ambiguous intelligence prompts', () => {
		...
		).toBe('Cross-case analysis: open Subjects & Assets in Intelligence mode (search & Ask): /case/abc-123/intelligence?mode=intelligence');
	});
	...
	it('returns handoff message only (no hybrid text)', () => {
		...
		expect(handoff).toBe('Cross-case analysis: open Subjects & Assets in Intelligence mode (search & Ask): /case/abc-123/intelligence?mode=intelligence');

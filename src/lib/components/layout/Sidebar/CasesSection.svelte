<script lang="ts">
	...
	async function loadCases() {
		...
		casesError = '';
		try {
			cases = await listCases($unitFilter, token);
		} catch (e: any) {
			...
			{:else}
				<div class="flex flex-col max-h-48 overflow-y-auto scrollbar-hidden space-y-0.5">
					{#each filteredCases as c (c.id)}
						<button
							type="button"
							class="text-left rounded px-2 py-1.5 text-xs hover:bg-gray-100 dark:hover:bg-gray-800 transition {c.id === $activeCaseId
